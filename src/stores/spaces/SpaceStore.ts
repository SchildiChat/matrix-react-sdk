...
export const getChildOrder = (
    order: string, ts: number, roomId: string, roomName: string): Array<Many<ListIteratee<unknown>>> => {
    ...
    public getChildren(spaceId: string): Room[] {
        ...
        return sortBy(childEvents, ev => {
            return getChildOrder(ev.getContent().order, ev.getTs(), ev.getStateKey(),
                this.matrixClient?.getRoom(ev.getStateKey())?.name);
        }).map(ev => {
    ...
    private updateNotificationStates = (spaces?: SpaceKey[]) => {
        ...
        spaces.forEach((s) => {
            ...
            this.getNotificationState(s).setRooms(visibleRooms.filter(room => {
                ...
                if (!this.showSpaceDMBadges && dmBadgeSpace && DMRoomMap.shared().getUserIdForRoomId(room.roomId)) {
