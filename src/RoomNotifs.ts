...
export function determineUnreadState(
    ...
    const markedUnread = markUnreadEnabled && isRoomMarkedAsUnread(room);
    ...
    // SC: Muted rooms may also be rendered "marked as unread" --> exclude
    if (getRoomNotifsState(room.client, room.roomId) === RoomNotifState.Mute && !markedUnread) {
        if (Unread.doesRoomHaveUnreadMessages(room)) {
            // SC: muted can still show unread counter
            return { symbol: null, count: 0, color: NotificationColor.Bold };
        } else {
